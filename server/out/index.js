...
var PouchFind = _interopDefault(require('pouchdb-find'));
...
function assignInDefaults(objValue, srcValue, key, object) {
  if (objValue === undefined ||
      (eq(objValue, objectProto$19[key]) && !hasOwnProperty$17.call(object, key))) {
    return srcValue;
  }
  return objValue;
}
...
const duration = Joi.string().regex(/^P(?!$)(\d+Y)?(\d+M)?(\d+W)?(\d+D)?(T(?=\d+[HMS])(\d+H)?(\d+M)?(\d+S)?)?$/, 'ISO 8601 Duration');
...
const columns = ['class', 'product', 'unit', 'valuePerUnit', 'salvageValue', 'lifeSpan', 'barcode', 'supplier', 'sku'];
 ...
 */
const id$1 = () => `_${Math.random().toString(36).substr(2, 9)}`;
...
function transformInputToRow(input) {
	...
	return Object.assign({ _id: itemAsset({ id: id$1(), product: snakeCase(input.product) }) }, input, {
